...
Lockfile syntax and patch consistency belong to pinned pnpm, not a repository-maintained parser or top-level key allowlist. `npm run bootstrap` uses `install --frozen-lockfile`, including in Docker. Supply-chain tests exercise the committed patches and reject patch drift and malformed lockfiles through that same pinned runner in disposable copies. The scanner's dependency-free preinstall checks cover manifests, scanner configuration, and exceptions; lockfile integrity and source restrictions are checked separately by `check:supply-chain` using the installed YAML parser. Run bootstrap and `check:supply-chain` before a standalone scan.
