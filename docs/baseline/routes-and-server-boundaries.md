...
| `/join` (`/signup` redirects)                  | Public                                                                    | Choose a Supporter, Member, or Solidarity tier and continue through Stripe Checkout         | tier/connection selection, redirect, validation/email, failure              |
...
R-020A makes existing-account passwordless login executable at `/login`; ordinary email and phone verification cannot create an unknown account. Issue #70 adds the only public account-creation path at `/join`, with `/signup` redirecting there: all three tiers use subscription-mode Stripe Checkout, the return path freshly validates the exact Price, tier, active Subscription, Customer, identifiers, and matching Checkout/Customer email, and a five-minute link sent to that Stripe email atomically creates or finds the account and claims the unique Customer and Subscription. The on-page Code of Conduct link is disclosure only, Supporter grants no paid rights, and both paid tiers grant the same paid-member access. Authenticated `/account` owns minimized identity and linked-provider display/unlinking, while R-021 owns global sign-out in the account menu; signed-out requests return to `/login`. The app-owned frontend `/auth` route is absent, while Better Auth's framework handler remains under `/api/auth/**`.
...
| `POST /api/join/checkout`                               | Public                                                      | Accept one tier, select its server-owned Price, and create subscription-mode Checkout with payment methods collected only if required.                                                                                                                           |
| `POST /api/join/complete`                               | Completed matching Stripe Checkout                          | Freshly validate the Session, Customer, Subscription, tier, Price, active status, identifiers, and email agreement, then send a short-lived account claim link to the Stripe email.                                                                              |
