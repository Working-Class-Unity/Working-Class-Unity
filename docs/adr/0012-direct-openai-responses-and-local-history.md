...
- Isolate the SDK in `server/services/ai/openai.ts`. The interface is intentionally OpenAI-specific and thin: it permits deterministic injection, validates the request/response boundary, normalizes safe errors, and prevents routes from handling provider envelopes. It is not a multi-provider abstraction.
