...
export function graceWindowFromFirstFailure(firstFailure: Date): Readonly<{ startedAt: string; endsAt: string }> {
  ...
  if (!Number.isFinite(firstFailureMs)) {
    throw new TypeError('First failure time must be valid')
  }
...
export function evaluateStripeSubscriptionAccess(
  ...
  if (snapshot.reconciliationRequired) {
    return reconciliationRequired('provider_projection_ambiguous')
  }
  ...
  if (snapshot.status === 'active') {
    if (snapshot.cancelAtPeriodEnd) {
      ...
      if (!Number.isFinite(currentPeriodEnd)) {
        return reconciliationRequired('missing_or_invalid_current_period_end')
      }
      if (now.getTime() >= currentPeriodEnd) {
        return reconciliationRequired('nonrenewing_period_ended')
      }
    }
