...
const billingDetachedSubscriptionCancellationSafetyLimit = 25
      ...
      `insert into job_queue (type, payload, max_attempts, run_after, created_at, updated_at)
       ...
       where not exists (
         ...
         where type = ?
           ...
           and json_extract(payload, '$.subjectId') = ?
        ...
        .prepare(
          `select subject.id
           ...
           where subject.provider = 'stripe'
             and subject.provider_reference like 'sub\\_%' escape '\\'
           ...
           order by subject.status_updated_at, subject.id
           limit ?`
        )
  ...
  return async (payload: JsonValue) => {
    ...
    if (!terminalStatuses.has(subscription.status)) {
      try {
        ...
        // The exact retrieve below converges a lost cancellation response.
      }
      try {
        subscription = await client.subscriptions.retrieve(authority.stripeSubscriptionId)
      } catch {
    ...
    const updated = connection.sqlite
      .prepare(
        `update detached_billing_subjects
         set provider_status = ?, status_updated_at = ?
         where id = ?
           and provider = 'stripe'
...
function readAuthority(connection: DatabaseConnection, subjectId: string): DetachedCancellationAuthority | null {
  const row = connection.sqlite
    .prepare(
      `select
         id,
         provider_reference as stripeSubscriptionId,
         provider_customer_reference as stripeCustomerId,
         provider_status as status
       from detached_billing_subjects
       where id = ?
         and provider = 'stripe'
         and provider_reference like 'sub\\_%' escape '\\'
