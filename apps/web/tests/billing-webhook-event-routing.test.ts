...
import { observeStripeWebhookCurrentState } from '../server/services/payments/billing-webhook-state'
...
const missingRoutingCases = [
  ...
  objectId: string
  providerKind: 'financial_risk' | 'invoice' | 'schedule' | 'subscription'
...
describe('Stripe webhook event routing contract', () => {
  ...
  it.each(missingRoutingCases)(
    ...
    async ({ eventType, objectId, providerKind, reads }) => {
      ...
      expect(isStripeWebhookEventType(eventType)).toBe(true)
      const observation = await observeStripeWebhookCurrentState(provider.client, config(), {
    ...
    subscriptions: {
      retrieve: vi.fn(async (id: string, parameters: unknown, options: unknown) => {
        ...
        if (
          ...
          'expand' in parameters &&
          Array.isArray(parameters.expand)
        ) {
      ...
      data: [
        {
          ...
          quantity: 1,
          price: {
            id: 'price_family_monthly_webhook_routing',
            object: 'price'
          },
          discounts: []
...
function config(): AppRuntimeConfig {
  return {
    ...
    stripe: {
      ...
      personalWeeklyPriceId: 'price_personal_weekly_webhook_routing',
      personalMonthlyPriceId: 'price_personal_monthly_webhook_routing',
      personalAnnualPriceId: 'price_personal_annual_webhook_routing',
      familyMonthlyPriceId: 'price_family_monthly_webhook_routing',
      familyAnnualPriceId: 'price_family_annual_webhook_routing'
    }
