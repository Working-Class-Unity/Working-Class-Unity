...
async function run() {
  ...
  console.log(
    'Container maintenance smoke passed: 5 successful maintenance jobs, 6 fail-closed cases, 2 non-root container generations, packaged credential-free off-host operator, persistent event database, and the healthy-to-unhealthy readiness transition.'
  )
}
...
async function writePersistentState() {
  await docker(
    [
      ...
      'node',
      '-e',
      `const Database=require('/app/.output/server/node_modules/better-sqlite3');const db=new Database('/app/data/app.db');db.prepare("insert into events (id,title,kind,visibility) values (?,?,'social','public')").run(${JSON.stringify(persistenceKey)},${JSON.stringify(persistenceValue)});db.close();`
    ],
    { label: 'persistent event write', timeout: 10_000 }
  )
}
...
async function mutatePersistentDatabase() {
  await docker(
    [
      ...
      'node',
      '-e',
      `const Database=require('/app/.output/server/node_modules/better-sqlite3');const db=new Database('/app/data/app.db');db.prepare('update events set title=? where id=?').run(${JSON.stringify(mutatedValue)},${JSON.stringify(persistenceKey)});db.close();`
    ],
...
async function assertDatabaseValue(expected, phase) {
  const result = await docker(
    [
      ...
      'node',
      '-e',
      `const Database=require('/app/.output/server/node_modules/better-sqlite3');const db=new Database('/app/data/app.db',{readonly:true});const row=db.prepare('select title from events where id=?').get(${JSON.stringify(persistenceKey)});db.close();process.stdout.write(row.title);`
    ],
...
async function assertPersistentState() {
  const result = await docker(
    [
      ...
      'node',
      '-e',
      `const Database=require('/app/.output/server/node_modules/better-sqlite3');const{existsSync,readdirSync}=require('node:fs');const db=new Database('/app/data/app.db',{readonly:true});const row=db.prepare('select title from events where id=?').get(${JSON.stringify(persistenceKey)});db.close();const backups=readdirSync('/app/data/backups');process.stdout.write(JSON.stringify({backups:existsSync(${JSON.stringify(knownGoodBackup)})&&backups.some((name)=>name.includes('pre-migrate'))&&backups.some((name)=>name.includes('pre-restore')),database:row.title}));`
    ],
    { label: 'replacement persistence inspection', timeout: 10_000 }
  )
  assert(
    isExactRecord(JSON.parse(result.stdout), { backups: true, database: persistenceValue }),
    'Event records and backups did not survive container replacement and restore'
  )
}
