...
2. Temporarily suspend the approved `master` auto-deploy trigger, the Stripe membership synchronization task, and process restart policies, then stop migration, web, worker, any enabled private backup or synchronization runner, and every other process that can open SQLite. Ensure no deployment or scheduled task can restart or overlap a writer. `--confirm-app-stopped` is an operator assertion, not automatic process discovery; do not supply it while any writer can restart.
...
8. After accepting the restored app state, start the Coolify Compose resource from the same selected Git commit. Its one-shot migration must verify and complete before web, the one default worker, and any enabled private backup or synchronization runner start against the shared `/app/data` volume. Resume the synchronization task only after restored Stripe state has been reconciled deliberately.
